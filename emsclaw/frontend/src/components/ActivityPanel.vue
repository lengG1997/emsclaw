          ...
          <div v-if="toolsExpanded" ref="toolsContentRef"
            ...
            <div class="px-3 py-2 bg-[var(--fill-tsp-gray-main)] rounded-lg">
              <div class="flex flex-col gap-0.5">
                ...
                <template v-for="batch in visibleToolBatches" :key="batch.id">
                  <!-- ── 并发 batch: 整体一个框(蓝底+边框+圆角+头部状态条) ── -->
                  <div v-if="batch.items.length > 1" class="my-1 rounded-lg border border-blue-300 dark:border-blue-700 bg-blue-100/70 dark:bg-blue-950/40 overflow-hidden">
                    <!-- batch 头部: 状态图标 + 标签 + 整批状态 -->
                    <div class="flex items-center gap-1.5 px-2.5 py-1 border-b border-blue-300/70 dark:border-blue-700/60 bg-blue-200/60 dark:bg-blue-900/50">
                      <div v-if="batchStatus(batch) === 'running'" class="relative size-3 flex-shrink-0">
                        <div class="absolute inset-0 rounded-full border-[1.5px] border-blue-300 dark:border-blue-700"></div>
                        <div class="absolute inset-0 rounded-full border-[1.5px] border-blue-600 border-t-transparent animate-spin"></div>
                      </div>
                      <div v-else class="size-3 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0">
                      ...
                      <span class="text-[10px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-300 flex items-center gap-1">
                        ...
                        并发 ×{{ batch.items.length }}
                      </span>
                      <span class="text-[9px] font-semibold text-blue-600 dark:text-blue-400 ml-auto">
                        {{ batchStatus(batch) === 'running' ? t('Running') : t('Done') }}
                      </span>
                    </div>
                    <!-- batch 工具列表(垂直堆叠, 每个可独立展开) -->
                    <div class="flex flex-col divide-y divide-blue-200/60 dark:divide-blue-800/50">
                      <template v-for="item in batch.items" :key="item.id">
                        <div v-if="item.tool" class="px-1.5">
                          <div
                            @click="toggleToolExpand(item.id)"
                            class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-blue-200/50 dark:hover:bg-blue-900/40 transition-colors cursor-pointer border border-transparent group/tool"
                            :class="{ 'bg-blue-200/60 dark:bg-blue-900/50 border-blue-300/60 dark:border-blue-700/50': expandedToolIds.has(item.id) }"
                          >
                            <ChevronRightIcon :size="10"
                              class="text-blue-400 dark:text-blue-500 transition-transform duration-150 flex-shrink-0"
                            ...
                            <div v-else class="flex-shrink-0 flex items-center justify-center w-4 h-4 text-blue-400 dark:text-blue-500">
                          ...
                          <div v-if="expandedToolIds.has(item.id)"
                            class="tool-detail-enter mt-1 mx-1 mb-1 flex flex-col gap-2 px-3 py-2.5 rounded-lg bg-white dark:bg-gray-900/60 border border-blue-200/50 dark:border-blue-800/40">
...
// 按 batch_id 对可见工具二次分组:同 batch 并排渲染,不同 batch 上下分隔。
// 无 batch_id 的工具各自成一组(保持原顺序行为)。
const visibleToolBatches = computed(() => {
  const batches: { id: string; items: typeof visibleToolItems.value }[] = [];
  for (const item of visibleToolItems.value) {
    if (!item.tool) continue;
    const bid = (item.tool as any).batch_id as string | undefined;
    const last = batches[batches.length - 1];
    if (bid && last && last.id === `batch:${bid}`) {
      last.items.push(item);
    } else {
      batches.push({ id: bid ? `batch:${bid}` : `solo:${item.id}`, items: [item] });
    }
  }
  return batches;
});
...
// 用于 batch 头部状态图标 + 边框配色。
const batchStatus = (batch: { items: typeof visibleToolItems.value }): 'running' | 'done' => {
  const tools = batch.items.map(i => i.tool).filter(Boolean);
  if (tools.some(t => t!.status === 'calling')) return 'running';
  return 'done';
};
