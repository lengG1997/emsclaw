<template>
  <div class="flex h-full transition-[width] duration-300 ease-in-out"
    :style="{ width: isLeftPanelShow ? '320px' : '76px' }">

    <!-- Navigation Rail -->
    <div class="nav-rail w-[76px] h-full flex flex-col items-center py-4 gap-2 border-r border-gray-200/60 dark:border-gray-700/40 z-20 flex-shrink-0 relative">
      <div class="absolute inset-0 bg-gray-50 dark:bg-[#111] -z-10"></div>

      <template v-for="g in MENU_CONFIG" :key="g.key">
        <!-- 顶层直链组（对话/总览）-->
        <button v-if="g.to && !g.children" @click="goDirect(g)"
          class="nav-btn w-[60px] py-2 rounded-xl flex flex-col items-center justify-center gap-1 transition-all duration-250 group relative"
          :class="activeGroupKey === g.key
            ? 'bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-md shadow-blue-500/25'
            : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-300'">
          <component :is="g.icon" :size="19" :stroke-width="activeGroupKey === g.key ? 2.5 : 1.8" />
          <span class="text-[10px] font-medium leading-none">{{ g.label }}</span>
          <div v-if="activeGroupKey === g.key" class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-7 bg-gradient-to-b from-blue-400 to-indigo-500 rounded-r-full"></div>
        </button>
        <!-- 分组组（平台/运营）-->
        <button v-else-if="g.children" @click="toggleGroup(g.key)"
          class="nav-btn w-[60px] py-2 rounded-xl flex flex-col items-center justify-center gap-1 transition-all duration-250 group relative"
          :class="activeGroupKey === g.key
            ? 'bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-700 text-white shadow-md shadow-violet-500/25'
            : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-300'">
          <component :is="g.icon" :size="19" :stroke-width="activeGroupKey === g.key ? 2.5 : 1.8" />
          <span class="text-[10px] font-medium leading-none">{{ g.label }}</span>
          <div v-if="activeGroupKey === g.key" class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-7 bg-gradient-to-b from-indigo-500 via-violet-500 to-fuchsia-600 rounded-r-full"></div>
        </button>
        <!-- 设置（action）-->
        <button v-else-if="g.action === 'settings'" @click="openSettingsDialog()"
          class="nav-btn w-[60px] py-2 rounded-xl flex flex-col items-center justify-center gap-1 transition-all duration-250 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-600 dark:hover:text-gray-300">
          <component :is="g.icon" :size="19" :stroke-width="1.8" />
          <span class="text-[10px] font-medium leading-none">{{ g.label }}</span>
        </button>
      </template>
    ...
    <div
      ...
      :class="isLeftPanelShow ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full w-0 pointer-events-none absolute left-[76px]'"
    >
      <div class="absolute inset-0 bg-white dark:bg-[#1a1a1a] -z-10"></div>

      <!-- 抽屉内容：关闭时立即隐藏内部，避免宽度/透明度过渡期间闪现旧内容（幻影）-->
      <div v-show="isLeftPanelShow" class="flex flex-col flex-1 min-h-0 overflow-hidden">

      <!-- ── 抽屉主体：三块互斥，会话列表只在对话组激活时显示 ── -->

      <!-- ① 二级菜单列表（平台/运营组展开时）-->
      <div v-if="expandedGroup && expandedGroup !== 'chat'" class="p-2 border-b border-[var(--border-main)]">
        <div class="text-[11px] text-[var(--text-tertiary)] px-2 py-1">{{ groupLabel(expandedGroup) }}</div>
        <button v-for="c in groupChildren(expandedGroup)" :key="c.key" @click="goChild(c)"
          class="w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-xs transition-colors"
          :class="activeChildKey === c.key ? 'bg-[var(--background-gray-main)] text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:bg-[var(--background-gray-main)]'">
      ...
      <!-- ② 会话列表抽屉（仅对话组激活时）-->
      <template v-else-if="activeGroupKey === 'chat'">
        <!-- Drawer Header - Minimal style -->
        <div class="flex items-center px-4 pt-4 pb-1 flex-shrink-0">
          <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ t('Chats') }}</h2>
          <span class="ml-2 text-xs text-gray-400 tabular-nums">{{ stats.all }}</span>
        </div>
        ...
        <!-- Filter Tabs (全部/置顶/...) -->
        <div class="px-3 mb-1 flex-shrink-0">
        ...
        <div class="px-3 mb-2 flex-shrink-0">
          <button @click="handleNewTaskClick()"
            ...
            <span class="whitespace-nowrap truncate">{{ t('New Task') }}</span>
            <div class="flex items-center gap-0.5 ml-auto opacity-60">
              <Command :size="11" />
      ...
      <!-- ③ 任务列表抽屉（仅任务页时）-->
      <template v-else-if="isTasksActive">
        <!-- Drawer Header -->
        <div class="flex items-center px-4 pt-4 pb-1 flex-shrink-0">
          <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ t('Scheduled Tasks') }}</h2>
          <span class="ml-2 text-xs text-gray-400 tabular-nums">{{ scheduledTasks.length }}</span>
        </div>

        <!-- New Scheduled Task Button -->
        <div class="px-3 mb-2 flex-shrink-0">
          <button @click="handleNewScheduledTaskClick()"
            class="new-task-btn flex w-full items-center justify-center gap-2 rounded-lg h-9 px-3 bg-gradient-to-r from-blue-400/90 to-indigo-500/90 dark:from-blue-500/80 dark:to-indigo-600/80 text-white font-medium text-sm cursor-pointer transition-all duration-150 hover:from-blue-500 hover:to-indigo-600 dark:hover:from-blue-500 dark:hover:to-indigo-600 hover:shadow-md hover:shadow-blue-400/20 active:scale-[0.98]">
            <Plus class="h-4 w-4 flex-shrink-0" />
            <span class="whitespace-nowrap truncate">{{ t('New Scheduled Task') }}</span>
          </button>
        </div>
      ...
      <!-- ④ 其他情况（平台/运营组收起、设置等）：抽屉空白，不显示会话列表 -->

      </div><!-- /v-show isLeftPanelShow 内容包层 -->
    </div>
  </div>
</template>
...
import {
  ...
  Play, ShieldCheck, LayoutDashboard, TrendingUp, BarChart3, Award, ListChecks, Bot, FlaskConical, BookOpen
...
// 二级菜单配置
type MenuChild = { key: string; label: string; to: string; icon: any; onGo?: () => void };
type MenuGroup = { key: string; label: string; icon: any; to?: string; children?: MenuChild[]; action?: 'settings' };

const MENU_CONFIG: MenuGroup[] = [
  { key: 'chat', label: '对话', icon: markRaw(MessageSquare), to: '/' },
  { key: 'overview', label: '总览', icon: markRaw(LayoutDashboard), to: '/chat/overview' },
  { key: 'intro', label: '介绍', icon: markRaw(BookOpen), to: '/chat/intro' },
  {
    key: 'platform', label: '平台', icon: markRaw(Blocks),
    children: [
      { key: 'agents', label: '智能体', to: '/chat/agents', icon: markRaw(Bot) },
      { key: 'skills', label: '技能', to: '/chat/skills', icon: markRaw(Blocks) },
      { key: 'tools', label: '工具', to: '/chat/tools', icon: markRaw(Wrench) },
      { key: 'tasks', label: '任务', to: '/chat/tasks', icon: markRaw(CalendarClock), onGo: fetchScheduledTasks },
      { key: 'approvals', label: '审批', to: '/chat/approvals', icon: markRaw(ShieldCheck) },
    ],
  },
  {
    key: 'ops', label: '运营', icon: markRaw(TrendingUp),
    children: [
      { key: 'model-overview', label: '模型', to: '/chat/model-overview', icon: markRaw(BarChart3) },
      { key: 'scores', label: '质量', to: '/chat/scores', icon: markRaw(Award) },
      { key: 'score-traces', label: '评分', to: '/chat/score-traces', icon: markRaw(ListChecks) },
      { key: 'experiments', label: '评估', to: '/chat/experiments', icon: markRaw(FlaskConical) },
    ],
  },
  { key: 'settings', label: '设置', icon: markRaw(Settings2), action: 'settings' },
];

// 二级菜单：哪个组展开（默认命中当前路由所在组）
const expandedGroup = ref<string | null>(null);

const activeChildKey = computed(() => {
  // 反查当前 route 命中哪个 child
  for (const g of MENU_CONFIG) {
    for (const c of g.children || []) {
      if (route.path.startsWith(c.to)) return c.key;
    }
  }
  return null;
});
const activeGroupKey = computed(() => {
  // 对话组：路径为 '/' 或 '/chat/<sessionId>' 且未命中任何子路由
  if (route.path === '/' || route.path.startsWith('/chat/session')) return 'chat';
  for (const g of MENU_CONFIG) {
    if (g.children?.some(c => route.path.startsWith(c.to))) return g.key;
    // 顶层直链（总览）；对话组 to='/' 会吞掉所有路径，跳过
    if (g.to && g.to !== '/' && route.path.startsWith(g.to)) return g.key;
  }
  // 未命中已知子路由的 /chat/* 视为会话页（sessionId 段），归到对话组
  if (route.path.startsWith('/chat/')) {
    const known = MENU_CONFIG.flatMap(g => [
      ...g.children?.map(c => c.to) || [],
      ...(g.to && g.to !== '/' ? [g.to] : []),
    ]);
    if (!known.some(to => route.path.startsWith(to))) return 'chat';
  }
  return null;
});
// 初始化展开命中组；切回对话组/顶层直链（无 children）时清空 expandedGroup，避免残留其他组的二级菜单
watch(activeGroupKey, (g) => {
  const grp = MENU_CONFIG.find(x => x.key === g);
  if (g && g !== 'chat' && grp?.children?.length) expandedGroup.value = g;
  else expandedGroup.value = null;
}, { immediate: true });

const goDirect = (g: MenuGroup) => {
  // 对话组：先清掉残留的 expandedGroup，否则抽屉里仍显示其他组的二级菜单
  if (g.key === 'chat') expandedGroup.value = null;
  // 非对话的顶层直链（总览）：无抽屉内容，直接跳转并收起抽屉
  if (g.key !== 'chat') {
    if (activeGroupKey.value !== g.key && g.to) router.push(g.to);
    if (isLeftPanelShow.value) toggleLeftPanel();
    return;
  }
  // 对话组：复用原 handleChatTabClick 行为
  if (activeGroupKey.value === g.key && isLeftPanelShow.value) { toggleLeftPanel(); return; }
  if (!isLeftPanelShow.value) toggleLeftPanel();
  if (activeGroupKey.value !== g.key && g.to) router.push(g.to);
};
const toggleGroup = (key: string) => {
  const group = MENU_CONFIG.find(g => g.key === key);
  const firstChild = group?.children?.[0];
  // 已展开该组 → 收起整个抽屉
  if (expandedGroup.value === key && isLeftPanelShow.value) {
    expandedGroup.value = null;
    toggleLeftPanel();
    return;
  }
  // 抽屉关着 → 打开抽屉、展开该组、并跳到第一个子菜单（如果当前不在该组任何子菜单上）
  if (!isLeftPanelShow.value) toggleLeftPanel();
  expandedGroup.value = key;
  if (firstChild && activeChildKey.value !== firstChild.key) {
    if (firstChild.onGo) firstChild.onGo();
    router.push(firstChild.to);
  }
};
const goChild = (c: MenuChild) => {
  if (!isLeftPanelShow.value) toggleLeftPanel();
  if (c.onGo) c.onGo();
  router.push(c.to);
};
const groupLabel = (key: string) => MENU_CONFIG.find(g => g.key === key)?.label || '';
const groupChildren = (key: string) => MENU_CONFIG.find(g => g.key === key)?.children || [];
...
watch(() => route.path, async (newPath, oldPath) => {
  const wasChatSession = oldPath?.startsWith('/chat/') && !oldPath.includes('skills') && !oldPath.includes('tools') && !oldPath.startsWith('/chat/tasks') && !oldPath.startsWith('/chat/agents')
  const isChatSession = newPath?.startsWith('/chat/') && !newPath.includes('skills') && !newPath.includes('tools') && !newPath.startsWith('/chat/tasks') && !newPath.startsWith('/chat/agents')
