    ...
    <div v-else class="flex flex-col gap-8">
      ...
      <div v-if="systemModels.length > 0" class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
                <!-- Header with Status -->
                <div class="absolute top-0 right-0 p-3 flex flex-col items-end gap-1.5">
                    <div v-if="model.id === defaultModelId" class="flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/30 dark:to-yellow-900/30 text-amber-600 dark:text-amber-400 rounded-full border border-amber-200 dark:border-amber-700/40 shadow-sm">
                        <Star class="size-3.5 fill-current" />
                        <span class="text-[10px] font-bold uppercase tracking-wide">{{ t('Default') }}</span>
                    </div>
        ...
        <div v-else class="grid grid-cols-1 gap-3">
            ...
            >
                ...
                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <button
                        class="p-2 text-[var(--text-secondary)] hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-all"
                        :title="t('Set as Default')"
                        :disabled="settingDefault === model.id"
                        @click="setAsDefault(model)"
                    >
                        <Loader2 v-if="settingDefault === model.id" class="size-4 animate-spin" />
...
import { Plus, Pencil, Trash2, Loader2, Box, CheckCircle2, ShieldCheck, Globe, Key, Radar, ChevronDown, Star } from 'lucide-vue-next';
import ProviderIcon from '../icons/ProviderIcon.vue';
import { 
  Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter 
} from '@/components/ui/dialog';
import {
  listModels, createModel, updateModel, deleteModel, detectContextWindow, setDefaultModel,
  type ModelConfig
} from '@/api/models';
...
// The "default" model is the first is_system entry — mirrors HomePage's selection logic
const defaultModelId = computed(() => systemModels.value[0]?.id ?? null);

const setAsDefault = async (model: ModelConfig) => {
  settingDefault.value = model.id;
  try {
    await setDefaultModel(model.id, true);
    showSuccessToast(t('Default model updated'));
    await fetchModels();
  } catch (err: any) {
    console.error(err);
