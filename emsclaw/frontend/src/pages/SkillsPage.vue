  ...
  <div class="flex flex-col h-full w-full overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-5 bg-[#f8f9fb] dark:bg-[#111]">
      ...
      <div v-if="skills.length === 0 && !loading" class="flex flex-col items-center justify-center h-full text-[var(--text-tertiary)] gap-3">
        ...
        <p class="text-xs opacity-60">Place skill packages under the Skills/ directory to install</p>
      </div>
      ...
      <!-- Skill Cards (按 Agent 分组) -->
      <div v-else>
        <section v-for="group in groups" :key="group.key" class="mb-7">
          <div class="flex items-center gap-2 mb-3 max-w-[1800px] mx-auto">
            <h2 class="text-xs font-semibold text-[var(--text-secondary)] tracking-wide whitespace-nowrap">{{ group.title }}</h2>
            <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-[var(--text-tertiary)]">{{ group.items.length }}</span>
            <div class="flex-1 h-px bg-gray-100 dark:bg-gray-800"></div>
          </div>
          <div v-if="group.items.length" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 max-w-[1800px] mx-auto">
            <div v-for="(skill, idx) in group.items" :key="skill.name"
              ...
              <div class="relative p-4">
                ...
                <p v-if="skill.description" class="text-xs text-[var(--text-secondary)] leading-relaxed line-clamp-2 mb-2">{{ skill.description }}</p>
                <!-- File list preview -->
                <div class="space-y-0.5 text-[10px] font-mono text-[var(--text-tertiary)] min-h-[2rem]">
          ...
          <p v-else class="text-xs text-[var(--text-tertiary)] opacity-60 ml-1 max-w-[1800px] mx-auto">(无)</p>
        </section>
      </div>

      <!-- Empty search -->
      <div v-if="!loading && skills.length > 0 && groups.length === 0" class="flex flex-col items-center justify-center py-20 gap-3">
        <div class="size-16 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
...
// 按 Agent 分组：无 domain 的归「外置/顶层内置」；有 domain 的按 domain 分组。
// 无搜索时外置组始终展示（空则显示"无"），搜索时仅展示有匹配的组。
const groups = computed<SkillGroup[]>(() => {
  const q = searchQuery.value.trim().toLowerCase();
  const searching = !!q;
  const items = skills.value.filter(s => !q || s.name.toLowerCase().includes(q));
  const externalItems = items.filter(s => !s.domain);
  const domainMap = new Map<string, ExternalSkillItem[]>();
  ...
  if (!searching || externalItems.length) {
    result.push({ key: '__external', title: '外置 / 顶层内置', items: externalItems });
  }
  for (const [domain, dItems] of [...domainMap.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    result.push({ key: domain, title: domain, items: dItems });
  }
  return result;
});
