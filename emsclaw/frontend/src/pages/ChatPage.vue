    ...
    <SimpleBar ref="simpleBarRef" @scroll="handleScroll">
      ...
      <div class="mx-auto w-full max-w-full sm:max-w-[768px] sm:min-w-[390px] flex flex-col flex-1">
        <div class="flex flex-col w-full gap-[12px] pb-[80px] pt-[12px] flex-1 overflow-y-auto">
          <!-- 快捷卡片:四方利益方一键发起调度规划。空闲时始终显示(不依赖"空会话",因为进会话必带消息/恢复历史) -->
          <QuickActionCards v-if="!isLoading" @select="handleSuggestionClick" class="pt-6" />
          <template v-for="(group, index) in groupedMessages" :key="group.id">
            ...
            <SubagentBlock v-else-if="group.type === 'single' && group.message && group.message.type === 'subagent'"
          ...
          <!-- HITL 审批卡片(business v2:pending approvals)-->
          <ApprovalCard
            v-for="a in pendingApprovals"
            :key="(a as any).interrupt_id || (a as any).event_id"
            :approval="a"
            :resuming="isResuming"
            :autoApprove="autoApprove"
            @submit="submitApproval"
            @toggle-auto="toggleAutoApprove"
          />
...
import { Message, MessageContent, ToolContent, StepContent, AttachmentsContent, ThinkingContent, SubagentContent } from '../types/message';
import {
  StepEventData,
  ToolEventData,
  MessageEventData,
...
import SubagentBlock from '../components/SubagentBlock.vue';
...
const createInitialState = () => ({
  ...
  // ── business v2:子 agent + HITL 审批状态 ──
  activeSubagentId: null as string | null, // 当前运行中的子 agent agent_id(顺序执行,至多一个)
  pendingApprovals: [] as ApprovalEventData[], // 待审批列表(kind=required,未 decided)
  autoApprove: false, // auto_approve_all 开关
  isResuming: false, // 审批 resume SSE 进行中
});
...
const handleToolEvent = (toolData: ToolEventData) => {
  ...
  // Detect sub-agent delegation via `task` tool (deepagents subagent dispatch)
  if (toolContent.function === 'task' && toolContent.args?.subagent_type) {
    toolContent.delegated_to = toolContent.args.subagent_type;
  }

  // Detect propose_skill_save on 'called' event.
...
// ── 子 agent 委派处理(agent_call/agent_result + 子 agent thinking/tool/plan 归属)──
// deepagents 顺序执行子 agent,同一时刻至多一个运行;事件带 agent_id/depth,depth>=1 路由到子 agent。
const findSubagentMessage = (agentId: string | null | undefined): Message | undefined => {
  if (!agentId) return undefined;
  return messages.value.find(
    m => m.type === 'subagent' && (m.content as SubagentContent).agent_id === agentId,
  );
};

// 当前活动子 agent message(activeSubagentId 优先,否则最后一个 running 的)
const getActiveSubagentMessage = (): Message | undefined => {
  if (activeSubagentId.value) {
    const m = findSubagentMessage(activeSubagentId.value);
    if (m) return m;
  }
  for (let i = messages.value.length - 1; i >= 0; i--) {
    const m = messages.value[i];
    if (m.type === 'subagent' && (m.content as SubagentContent).status === 'running') return m;
  }
  return undefined;
};

// agent 事件:call=子 agent 开始;result=子 agent 结束
const handleAgentEvent = (data: AgentEventData) => {
  if (data.kind === 'call') {
    // 去重:resume 时 task 工具 on_tool_start 会再次触发同 agent_id 的 call
    // (interrupt 后恢复,checkpointer 重放 task 节点)。不重复建块,只重置为 running。
    const existing = findSubagentMessage(data.agent_id);
    if (existing) {
      const c = existing.content as SubagentContent;
      c.status = 'running';
      if (data.description) c.input = data.description;
      activeSubagentId.value = data.agent_id;
      return;
    }
    const sub: SubagentContent = {
      timestamp: data.timestamp || 0,
      agent_id: data.agent_id,
      subagent_type: data.subagent_type,
      name: data.subagent_type,
    ...
    messages.value.push({ type: 'subagent', content: sub });
    activeSubagentId.value = data.agent_id;
  } else if (data.kind === 'result') {
    // mark ALL matching blocks complete(防御性:即便有重复块也全部收尾)
    for (const m of messages.value) {
      if (m.type === 'subagent' && (m.content as SubagentContent).agent_id === data.agent_id) {
        const c = m.content as SubagentContent;
        c.status = 'complete';
        c.result = data.result || '';
      }
    }
    if (activeSubagentId.value === data.agent_id) {
      activeSubagentId.value = null;
    }
  }
};

// 子 agent thinking 累积
const routeThinkingToSubagent = (agentId: string | null | undefined, content: string) => {
  const m = (agentId ? findSubagentMessage(agentId) : null) || getActiveSubagentMessage();
  if (m && content) {
    (m.content as SubagentContent).thinking += content;
  }
};

// 子 agent 工具调用(复用 ToolContent,smartMerge 同 tool_call_id)
const routeToolToSubagent = (agentId: string | null | undefined, toolData: ToolEventData) => {
  const m = (agentId ? findSubagentMessage(agentId) : null) || getActiveSubagentMessage();
  if (!m) return;
  const c = m.content as SubagentContent;
  const toolContent = { ...toolData } as ToolContent;
  if (typeof toolContent.content === 'string') {
    try { toolContent.content = JSON.parse(toolContent.content); } catch (_) { /* keep string */ }
  }
  const existing = c.tools.find(t => t.tool_call_id === toolContent.tool_call_id);
  if (existing) {
    smartMerge(existing, toolContent);
  } else {
    c.tools.push(toolContent);
  }
};

// 子 agent plan/todo
const routePlanToSubagent = (agentId: string | null | undefined, planData: PlanEventData) => {
  const m = (agentId ? findSubagentMessage(agentId) : null) || getActiveSubagentMessage();
  if (!m) return;
  const c = m.content as SubagentContent;
  c.plan = (planData.steps || []).map(s => ({ ...s, tools: [] }) as StepContent);
};

// ── HITL 审批处理 ──
const handleApprovalEvent = (data: ApprovalEventData) => {
  if (data.kind === 'required') {
    const iid = data.interrupt_id || '';
    if (iid && !pendingApprovals.value.some(a => (a.interrupt_id || '') === iid)) {
      pendingApprovals.value.push({ ...data });
    }
  } else if (data.kind === 'decided') {
    // 决策已记录 -> 从待审批移除(runner B3 resume 首个事件;stream 继续)
    const iid = data.interrupt_id || '';
    if (iid) {
      pendingApprovals.value = pendingApprovals.value.filter(a => (a.interrupt_id || '') !== iid);
    }
  } else if (data.kind === 'auto_approve_set') {
    autoApprove.value = data.enabled !== false;
  }
};

// 用户点 approve/reject -> resume SSE,流剩余事件喂回 handleEvent
const submitApproval = async (interruptId: string, decision: string) => {
  ...
  const resumeSessionId = sessionId.value;
  const isStale = () => _unmounted || resumeSessionId !== sessionId.value;
  ...
  const resetSSETimer = () => {
    ...
    sseTimer = setTimeout(() => {
      if (isStale()) return;
      console.warn('Resume SSE inactivity timeout, force closing');
      isLoading.value = false;
      isResuming.value = false;
      if (cancelCurrentChat.value) { cancelCurrentChat.value(); cancelCurrentChat.value = null; }
    }, SSE_TIMEOUT_MS);
  };
  const clearSSETimer = () => { if (sseTimer) { clearTimeout(sseTimer); sseTimer = null; } };

  resetSSETimer();
  try {
    const cancelFn = await agentApi.resumeApproval(
      resumeSessionId, interruptId, { decision },
      {
        onOpen: () => { if (isStale()) return; resetSSETimer(); },
        onMessage: ({ event, data }) => {
          if (isStale()) return;
          resetSSETimer();
          try {
            handleEvent({ event: event as AgentSSEEvent['event'], data: data as AgentSSEEvent['data'] });
          } catch (e) { console.error('resume handleEvent error:', e); }
        },
        onClose: () => {
          clearSSETimer();
          if (isStale()) return;
...
// 开 auto_approve_all:设 flag(后端 resume=false);若有 pending 审批,前端驱动首个 resume(approve)
const toggleAutoApprove = async () => {
  if (!sessionId.value || _unmounted || autoApprove.value) return;
  try {
    await agentApi.setAutoApproveAll(sessionId.value, false);
    autoApprove.value = true;
    // 若有 pending 审批,自动 approve 驱动 resume
    // (auto_approve 已开,后续 interrupt 由 runner B4 在同一 stream 内自动 approve)
    if (pendingApprovals.value.length > 0) {
      const iid = pendingApprovals.value[0].interrupt_id || '';
      if (iid) await submitApproval(iid, 'approve');
    }
  } catch (e) {
    console.error('setAutoApproveAll failed:', e);
  }
};
...
const handleEvent = (event: AgentSSEEvent) => {
  ...
  if (event.event === 'message_chunk') {
    ...
    const td = event.data as ToolEventData;
    if (td.depth && td.depth >= 1) {
      routeToolToSubagent(td.agent_id, td);
    } else {
      handleToolEvent(td);
    }
    ...
    const td = event.data as ThinkingEventData;
    if (td.depth && td.depth >= 1) {
      routeThinkingToSubagent(td.agent_id, td.content);
    } else {
      handleThinkingEvent(td);
    }
    ...
    const pd = event.data as PlanEventData;
    if (pd.depth && pd.depth >= 1) {
      routePlanToSubagent(pd.agent_id, pd);
    } else {
      handlePlanEvent(pd);
    }
  } else if (event.event === 'agent') {
    handleAgentEvent(event.data as AgentEventData);
  } else if (event.event === 'approval') {
    handleApprovalEvent(event.data as ApprovalEventData);
  } else if (event.event === 'dispatch_preview') {
    // 调度策略求解成功事件(完整 schedule+summary,独立于 message 流)。
    // 渲染由后端 finally 兜底追加的 <dispatch_preview> 块 + ChatMessage 文本解析完成,
    // 此分支静默接收事件(可选用于显示 loading/预通知,初版不处理)。
    // P1 根治:只要 optimize_dispatch 返回 ok,前端必然收到图表数据(后端兜底保证)。
  }
...
// Switch conversation when route param changes (same component instance is reused,
// so onMounted won't fire again — without this, clicking a session in the list
// doesn't load the new conversation's history).
watch(() => router.currentRoute.value.params.sessionId, async (newId, oldId) => {
  if (!newId || newId === oldId) return;
  console.log('[ChatPage] route sessionId changed:', oldId, '->', newId);
  if (cancelCurrentChat.value) {
    cancelCurrentChat.value();
    cancelCurrentChat.value = null;
  }
