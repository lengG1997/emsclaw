...
参考 sample/sse_protocol_manager.py 实现，适配 emsclaw 实际工具集。
...
# 权威枚举集中在 entity.enums；本模块 re-export EventType 仅为向后兼容
# 已有 `from emsclaw_backend.deepagent.sse_protocol import EventType` 的调用点。
from emsclaw_backend.entity.enums import EventType  # noqa: F401  (re-export)
...
class ToolRegistry:
    ...
    def _initialize_default_tools(self):
        ...
        # ── EMS 场站数据工具（StationDataExpert） ──
        self.register(ToolMeta("get_station_overview", ToolCategory.DATA, "📊", "场站全量总览"))
        self.register(ToolMeta("get_storage_status", ToolCategory.DATA, "🔋", "储能状态"))
        self.register(ToolMeta("get_pv_status", ToolCategory.DATA, "☀️", "光伏状态"))
        self.register(ToolMeta("get_meter_status", ToolCategory.DATA, "🔌", "关口表状态"))
        self.register(ToolMeta("get_demand_status", ToolCategory.DATA, "📉", "需量状态"))
        self.register(ToolMeta("get_tariff", ToolCategory.DATA, "💴", "电价时段"))
        self.register(ToolMeta("get_daily_energy", ToolCategory.DATA, "📅", "今日电量"))
        self.register(ToolMeta("get_charge_schedule", ToolCategory.DATA, "🗓️", "充放电计划"))
        self.register(ToolMeta("get_forecast", ToolCategory.DATA, "📈", "负荷·光伏预测"))
        self.register(ToolMeta("get_storage_history", ToolCategory.DATA, "🔋", "储能历史"))
        self.register(ToolMeta("get_meter_history", ToolCategory.DATA, "🔌", "关口表历史"))
        self.register(ToolMeta("get_daily_energy_history", ToolCategory.DATA, "📅", "日电量历史"))

        # ── EMS 调度规划工具（DispatchPlanningExpert） ──
        self.register(ToolMeta("optimize_dispatch", ToolCategory.DATA, "🧮", "优化调度"))
        self.register(ToolMeta("apply_schedule", ToolCategory.EXECUTION, "⚡", "下发计划"))
        self.register(ToolMeta("account_revenue", ToolCategory.DATA, "💰", "收益核算"))
        self.register(ToolMeta("get_schedule_status", ToolCategory.DATA, "📈", "执行监测"))

        # ── EMS 设备操作工具（DeviceOperationExpert） ──
        self.register(ToolMeta("create_device", ToolCategory.SYSTEM, "📥", "录入设备"))
        self.register(ToolMeta("configure_network", ToolCategory.SYSTEM, "🌐", "配置网络"))
        self.register(ToolMeta("list_devices", ToolCategory.SYSTEM, "📋", "设备列表"))
        self.register(ToolMeta("get_device", ToolCategory.SYSTEM, "🔎", "查询设备"))
        self.register(ToolMeta("reset_station_defaults", ToolCategory.SYSTEM, "♻️", "重置场站"))
        self.register(ToolMeta("execute_device_command", ToolCategory.EXECUTION, "⚡", "设备指令"))
        self.register(ToolMeta("emergency_stop", ToolCategory.EXECUTION, "🚨", "紧急停止"))
