...
_KNOWN_CONTEXT_WINDOWS: list[tuple[str, int]] = [
    ...
    ("deepseekv3.2",            131_072),   # 部分网关的别名写法（无连字符）
...
class _SafeChatOpenAI(ChatOpenAI):
    ...
    def _is_stream_options_error(exc: BaseException) -> bool:
        """部分 OpenAI 兼容网关（如 aihubmix）不支持 stream_options.include_usage，
        会返回 400 Unknown parameter。"""
        text = str(exc)
        return "stream_options" in text or "include_usage" in text

    def _strip_stream_options(self, kwargs: dict) -> None:
        """从调用参数和 model_kwargs 中移除 stream_options，供降级重试使用。"""
        popped = kwargs.pop("stream_options", None)
        mk = getattr(self, "model_kwargs", None)
        if isinstance(mk, dict):
            popped = mk.pop("stream_options", None) or popped
        if popped is not None:
            logger.warning(
                "[engine] provider rejected stream_options.include_usage "
                "(model=%s, base_url=%s); retrying without token usage stats",
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]] = None, run_manager: Any = None, **kwargs: Any):  # type: ignore[override]
        ...
        try:
            ...
            if self._is_stream_options_error(e):
                self._strip_stream_options(kwargs)
                return super()._generate(messages, stop=stop, run_manager=run_manager, **kwargs)
            if isinstance(e, ValueError) and "No generations found in stream" in str(e):
                logger.warning(
    ...
    async def _astream(self, *args: Any, **kwargs: Any) -> Any:  # type: ignore[override]
        args, kwargs = self._sanitize_messages(args, kwargs)
        if "stream_options" not in kwargs and "stream_options" not in (getattr(self, "model_kwargs", None) or {}):
...
def get_llm_model(
    ...
    # 输出预算封顶:逐用户 task_settings.max_tokens 上限 200000,一次对话可产生
    # ~200k 输出 token(实测 trace 199680)导致计费失控。统一钳制到全局 cap,
    # 用户想提高输出质量可调 MAX_OUTPUT_TOKENS_CAP。
    if effective_max_tokens > settings.max_output_tokens_cap:
        logger.info(
            f"[engine] clamp max_tokens {effective_max_tokens} -> "
            f"{settings.max_output_tokens_cap} (cap MAX_OUTPUT_TOKENS_CAP)"
        )
        effective_max_tokens = settings.max_output_tokens_cap
