"""
DeepAgents 内置工具集 — 元能力与评测工具。

包含 propose_skill_save（用户库持久化入口）与
eval_skill / grade_eval（skill 评测）。
"""
...
def eval_skill(workspace_dir: str, skill_name: str, test_cases_json: str, iteration: int = 1) -> str:
    ...
    from emsclaw_backend.observability.eval.runner import run_eval_task, EvalResult
    import shortuuid
