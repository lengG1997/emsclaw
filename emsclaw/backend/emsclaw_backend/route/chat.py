...
from emsclaw_backend.deepagent.sessions import async_create_agent_session
...
async def task_chat(
    ...
    try:
        async for evt in arun_agent_task_stream(session, prompt):
...
async def _resolve_any_model_config() -> Optional[dict]:
    ...
    async with AsyncSessionLocal() as s:
        # api_key is NOT empty AND NOT null (NULL != '' is NULL/falsey in SQL,
        # so this single predicate excludes both)
        q = select(ModelRow).where(
            ModelRow.is_active == True,
            ModelRow.api_key != "",
        ).order_by(ModelRow.created_at.desc()).limit(1)
