...
from emsclaw_backend.im.lark_long_connection import LarkLongConnectionService
from emsclaw_backend.im.orchestrator import IMServiceOrchestrator
from emsclaw_backend.im.system_settings import (
    IMSystemSettings,
...
from emsclaw_backend.im.user_binding_service import IMUserBindingService
from emsclaw_backend.im.wechat_bridge import WeChatBridge
from emsclaw_backend.user.dependencies import User, require_user
...
def _build_orchestrator(im_settings: IMSystemSettings) -> IMServiceOrchestrator:
    ...
    if im_settings.lark_enabled and im_settings.lark_app_id and im_settings.lark_app_secret:
        from emsclaw_backend.im.adapters.lark import LarkAdapter, LarkMessageFormatter
...
async def unbind_lark_user(current_user: User = Depends(require_user)):
    removed = await _binding_service.unbind_lark_user(agent_user_id=current_user.id)
...
async def get_lark_bind_status(current_user: User = Depends(require_user)):
    binding = await _binding_service.get_lark_binding_status(agent_user_id=current_user.id)
    if not binding:
