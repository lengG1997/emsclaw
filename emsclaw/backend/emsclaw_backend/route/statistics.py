...
async def _aggregate_statistics(
    ...
    async with AsyncSessionLocal() as s:
        q = select(SessionRow).where(
            ...
            SessionRow.updated_at >= start_ts,
            SessionRow.updated_at <= end_ts,
        )
        sessions = (await s.execute(q)).scalars().all()
    ...
    for session in sessions:
        ...
        # 获取模型配置 - 支持多种字段名
        model_config = session.model_config_ or {}
        ...
        # 如果 model_config 为空，尝试从其他字段获取（SessionRow 无 model/model_id 列，保持兼容）
        if not model_name:
            model_name = getattr(session, "model", "") or getattr(session, "model_id", "")
...
async def get_statistics_sessions(
    ...
    try:
        ...
        async with AsyncSessionLocal() as s:
            # 获取总数
            count_q = select(func.count()).select_from(SessionRow).where(
                SessionRow.user_id == current_user.id,
                SessionRow.updated_at >= start_ts,
                SessionRow.updated_at <= end_ts,
            )
            ...
            rows_q = select(SessionRow).where(
                SessionRow.user_id == current_user.id,
                SessionRow.updated_at >= start_ts,
                SessionRow.updated_at <= end_ts,
            ).order_by(SessionRow.updated_at.desc()).offset(skip).limit(page_size)
            sessions = (await s.execute(rows_q)).scalars().all()
        ...
        for session in sessions:
            ...
            if not title:
                # 从第一条用户消息提取标题（events 中 role=user 的 message）
                for evt in events:
                    if evt.get("event") == "message" and (evt.get("data") or {}).get("role") == "user":
                        content = (evt.get("data") or {}).get("content", "")
                        title = content[:50] + "..." if len(content) > 50 else content
                        break

            # 格式化创建时间
            created_at_ts = session.created_at or 0
            if created_at_ts:
