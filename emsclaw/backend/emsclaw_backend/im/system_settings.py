...
from emsclaw_backend.db.models import IMSystemSetting
...
async def get_im_system_settings() -> IMSystemSettings:
    async with AsyncSessionLocal() as s:
        row = await s.get(IMSystemSetting, IM_SETTINGS_DOC_ID)
        if row and isinstance(row.payload, dict):
            return IMSystemSettings(**row.payload)
        defaults = get_env_default_im_settings()
        s.add(
            IMSystemSetting(
                id=IM_SETTINGS_DOC_ID,
