...
from emsclaw_backend.deepagent.sessions import async_get_agent_session
from emsclaw_backend.db.models import WeChatBridgeState
from emsclaw_backend.db.session import AsyncSessionLocal
from emsclaw_backend.im.base import IMPlatform
from emsclaw_backend.im.session_manager import IMSessionManager
from emsclaw_backend.im.user_binding import IMUserBindingManager
from emsclaw_backend.route.sessions import _agent_background_worker

WEIXIN_BASE_URL = "https://ilinkai.weixin.qq.com"
CHANNEL_VERSION = "emsclaw-1.0.0"
BOT_TYPE = "3"
...
class WeChatBridge:
    ...
    async def _save_state(self) -> None:
        ...
        async with AsyncSessionLocal() as s:
            row = await s.get(WeChatBridgeState, WECHAT_STATE_DOC_ID)
            if row:
                row.payload = payload
    ...
    async def _load_state(self) -> bool:
        ...
        if not row or not isinstance(row.payload, dict):
            return False
        doc = row.payload
        if not doc.get("bot_token"):
            return False
        self._bot_token = doc.get("bot_token")
    ...
    async def _clear_state(self) -> None:
        async with AsyncSessionLocal() as s:
            await s.execute(delete(WeChatBridgeState).where(WeChatBridgeState.id == WECHAT_STATE_DOC_ID))
            await s.commit()
    ...
    async def _process_message(self, msg: dict) -> None:
        ...
        if not binding:
            binding = await self._binding_mgr.create_binding(
                platform=IMPlatform.WECHAT,
                platform_user_id=from_user,
                agent_user_id=self._admin_user_id,
            )
        ...
        session = await self._session_mgr.get_or_create_session(
            ...
            user_id=binding.agent_user_id,
        )

        await self._send_typing(from_user, ctx_token)

        try:
            sci_session = await async_get_agent_session(session.agent_session_id)
            ...
            await _agent_background_worker(
                sci_session, session.agent_session_id, text, [],
            )
