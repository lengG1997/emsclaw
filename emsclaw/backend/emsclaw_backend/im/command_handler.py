...
from emsclaw_backend.im.base import IMMessage, IMMessageFormatter, IMResponse
    ...
    async def handle(
        ...
        if cmd == "/new":
            ...
            return CommandResult(
                response=IMResponse(
                    content_type="text",
                    content=f"已创建新会话：{created.agent_session_id}",
                ),
                should_stop=True,
            )
        ...
        if cmd == "/history":
            ...
            if not sessions:
                ...
                text = "最近会话：\n" + "\n".join([f"- {s.agent_session_id}" for s in sessions])
            return CommandResult(
        ...
        if cmd == "/status":
            ...
            content = "当前无活跃会话"
            if latest:
                content = f"当前会话：{latest.agent_session_id}"
            return CommandResult(
