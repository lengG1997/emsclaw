...
from emsclaw_backend.db.models import IMessageDedup
from emsclaw_backend.db.session import AsyncSessionLocal
from emsclaw_backend.im.base import IMAdapter, IMMessage, IMMessageFormatter, IMPlatform, IMResponse
from emsclaw_backend.im.command_handler import IMCommandHandler
...
class IMServiceOrchestrator:
    def __init__(
        ...
        self.dedup_model = IMessageDedup
    ...
    async def _mark_message_once(self, message: IMMessage) -> bool:
        async with AsyncSessionLocal() as s:
            try:
                s.add(
                    self.dedup_model(
                        id=f"{message.platform.value}:{message.message_id}",
    ...
    async def _process_message(self, adapter: IMAdapter, message: IMMessage):
        ...
        try:
            ...
            logger.info(
                ...
                f"session_id={session.agent_session_id}, message_id={message.message_id}"
            )
    ...
    async def _execute_ai_task(
        ...
        async for evt in arun_agent_task_stream(session=session, query=query):
