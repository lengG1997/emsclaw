...
from emsclaw_backend.im.user_binding import IMUserBinding, IMUserBindingManager
...
class IMUserBindingService:
    ...
    async def get_lark_binding_status(self, agent_user_id: str) -> Optional[IMUserBinding]:
        return await self.binding_repo.get_binding_by_agent_user(
            platform=IMPlatform.LARK,
            agent_user_id=agent_user_id,
        )

    async def unbind_lark_user(self, agent_user_id: str) -> bool:
        return await self.binding_repo.remove_binding_by_agent_user(
            platform=IMPlatform.LARK,
            agent_user_id=agent_user_id,
        )
